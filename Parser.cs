...
//			}
			...
			if (typeIdent != null) {
				var sth = symbolTable.GetSymbol(typeIdent.value);

				if (sth is ExternalMethodSymbol)
				{
					List<FormalParamSymbol> formalParams = new List<FormalParamSymbol>();
					//formalParams.Add(new FormalParamSymbol(new IdentToken(-1,-1,"somethingAmazing"), typeof(System.String),))
					MethodSymbol mi = new MethodSymbol(
						new IdentToken(-1, -1, "writeLine"),
						typeof(System.Nullable),
						formalParams.ToArray(), 
						(sth as ExternalMethodSymbol).methodInfo[0]);

					String methodIdent = "writeLine" + i;

                    Console.WriteLine((sth as ExternalMethodSymbol).methodInfo[i]);

					i++;
					emit.AddMethodCall((sth as ExternalMethodSymbol).methodInfo[i]);
				}
                TypeSymbol ts = sth as TypeSymbol;
				if (ts != null)
					type = ts.type;
				else
